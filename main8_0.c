...
void system_run(void){
	...
	//s2 indicates there is solution left/any at all
	}
}
...
void tap_amount(void){
	...
	char flow_holder;
		...
		count=0;
		req_delay_flow=0;
		uart_msg("Close tap"); //relies on user closing tap (if time use flow meter to check tap is off)
	    uart_nl();
		auto_valve(7); //closes valve 7
		filter_pump();
}
...
void flow_rate(void){
	flowrate=count*0.704;	 //counted pules multipled by 0.704ml/pulse		 //count=300
...
void flow_delay(int flow_num){
	if (flow_num==1){
		 minutes=tank_amount1/flowrate; 	 //tank_amount==500		   minutes=0.0394570
		...
		minutes=tank_amount2/flowrate; 			//tank_amount==1500		req_delay_flow=7102.27 (7 seconds)
		req_delay_flow=minutes*tank_factor;
		}
	else if(flow_num==3){
		minutes=tank_amount3/flowrate; 			//tank_amount==2500	   	req_delay_flow=11837.12 (12 seconds)
		req_delay_flow=minutes*tank_factor;
		}
	else{
...
///////////////////Reset Drive LEDS so motor doesnt overheat///////////////////
void rst_drive(void){
	 drive_led1=0;
	 drive_led2=0;
	 drive_led3=0;
...
void msec_delay(unsigned int  req_delay)	//msec delay
    {
      ...
      for (p = 0; p < req_delay; p = p +1)
        for (q = 0; q < 120; q = q +1);  //1275 from iteration --- 1 ms 
